...
function wp_rp_generate_related_posts_list_items($related_posts, $selected_related_posts, $post_categories) {
	...
	for ($i = 0; $i < $limit; $i++) {
		...
		if ($platform_options["display_publish_date"]) {
			...
			$output .= '<em class="wp_rp_publish_date">' . mysql2date($dateformat, $related_post->post_date) . '</em> ';
			//$output .= mysql2date($dateformat, $related_post->post_date) . " -- ";
		}

		$output .= '<h3><a href="' . $post_url . '" class="wp_rp_title">' . wptexturize($related_post->post_title) . '</a></h3>';
		...
		if ($platform_options["display_excerpt"]){
			...
			if ($excerpt) {
				...
				$output .= ' <p class="wp_rp_excerpt">' . $excerpt . '</p>';
			}
		}
...
function wp_rp_get_related_posts($before_title = '', $after_title = '') {
	...
	$related_posts_ul = '<section class="' . $css_classes . ' latest-post-selection one-columns">' . $related_posts_lis . '</section>';

	$related_posts_title = $title ? ($before_title ? $before_title . $title . $after_title : '<div class="home-blog-title"><h2 class="related_post_title">' . $title . '</h2></div>') : '';
