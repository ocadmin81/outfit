...
if(isset( $_POST['postTitle'] )) {
	...
	if(
		...
		if (empty($postTitle)) {
			$postTitleError =  esc_html__( 'יש להכניס כותרת', 'outfit-standalone' );
			$hasError = true;
		}
		if (empty($_POST['postCategory'])) {
			$catError = esc_html__( 'יש לבחור קטגוריה', 'outfit-standalone' );
			$hasError = true;
		}
		//Image Count check//
		...
		if($count == 0){
			$imageError = esc_html__( 'יש לעלות לפחות תמונה אחת', 'outfit-standalone' );
			$hasError = true;
		}
		//Image Count check//
						...
						<!-- add photos and media -->						
							...
							<div class="ad-categories">
								<div class="form-group post-cat-container">
									...
									<div class="item">
										<select id="category" name="postCategory" class="reg form-control form-control-md category-select" required>
											<option value="" selected><?php esc_html_e('בחר/י קטגוריה', 'outfit-standalone'); ?></option>
											<?php
								...
								<div class="form-group post-sub-cat-container" style="display: none;">
									...
									<div class="item">
										<select id="subcategory" name="postSubcategory" class="reg form-control form-control-md">
											<option value=""><?php esc_html_e('בחר/י תת קטגוריה', 'outfit-standalone'); ?></option>
								...
								<div class="form-group post-colors-container" style="display: none;">
									...
									<div class="item">
										<select id="color" name="postColor[]" class="form-control form-control-md" multiple>
											<option value=""><?php esc_html_e('בחר/י צבע', 'outfit-standalone'); ?></option>
											<?php
											foreach ($colors as $c): ?>
								...
								<div class="form-group post-age-groups-container" style="display: none;">
									<label class="text-left flip"><?php esc_html_e('גיל', 'outfit-standalone') ?> <span>*</span> </label>
									<div class="item">
										<select id="ageGroup" name="postAgeGroup[]" class="form-control form-control-md" multiple>
											<option value=""><?php esc_html_e('בחר/י גיל', 'outfit-standalone'); ?></option>
											<?php
								...
								<div class="form-group post-brands-container" style="display: none;">
									...
									<div class="item">
										<select id="brand" name="postBrand[]" class="form-control form-control-md" multiple>
											<option value=""><?php esc_html_e('בחר/י מותג', 'outfit-standalone'); ?></option>
											<?php
											foreach ($brands as $c): ?>
												<option value="<?php echo $c->term_id; ?>"><?php esc_html_e($c->name); ?></option>
											<?php endforeach; ?>
								...
								<div class="form-group post-conditions-container" style="display: none;">
									...
									<div class="item">
										<select id="condition" name="postCondition" class="reg form-control form-control-md">
											<option value=""><?php esc_html_e('בחר/י מצב פריט', 'outfit-standalone'); ?></option>
											<?php
							...
							<div class="form-group">
								...
								<div class="item">
									<textarea name="postContent" id="description" class="form-control" data-error="<?php esc_html_e('יש לכתוב תיאור', 'outfit-standalone') ?>"></textarea>
							...
							<!--Address-->
							<div class="form-group">
								<label class="text-left flip"><?php esc_html_e('נקודת איסוף עיקרית', 'classiera'); ?> <span>*</span></label>
								<div class="item">
									<input class="address" id="address" type="text" name="address" class="form-control form-control-md" placeholder="<?php esc_html_e('כתובת או עיר', 'classiera') ?>" required>
							...
							<div class="form-group">
								<label class="text-left flip"><?php esc_html_e('נקודת איסוף נוספת (לא חובה)', 'classiera'); ?> </label>
								<div class="item">
									<input class="address" id="address_2" type="text" name="address_2" class="form-control form-control-md" placeholder="<?php esc_html_e('כתובת או עיר', 'classiera') ?>">
							...
							<div class="form-group">
								...
								<div class="item">
									<input type="text" id="phone" name="postPhone" class="form-control form-control-md" required>
								</div>
							</div>
